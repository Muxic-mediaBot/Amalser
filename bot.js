...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        if (config.AFPLK == 'Nihad🖤' || config.AFPLK == 'afamk' || config.AFPLK == 'vava' || config.AFPLK == 'Nihad🖤') {
        //thanks to afnanplk
        console.log(
            chalk.green.bold('thanks for watching -key cofirmed-')
        );
         }
         else if (config.AFPLK !== 'Nihad🖤' || config.AFPLK !== 'afamk' || config.AFPLK !== 'vava' || config.AFPLK !== 'Nihad🖤') {
         console.log(
    ...
    setInterval(async () => { 
        ...
        while (getGMTh == 7 && getGMTm == 01) {
            return await conn.sendMessage(conn.user.jid, '*[ DAILY ANNOUNCEMENTS Nihad🖤 ]*\n\n' + announce, MessageType.text);
...
*/
                        ...
                        try {
                            ...
                            if (config.NOLOG === 'off') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ Nihad🖤 ~______~*' +
